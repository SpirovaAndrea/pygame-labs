...
GREEN     = (  0, 255,   0)
DARKGREEN = (  0, 155,   0)
BLUE      = (137, 207, 240) #1st req: color for the 2nd worm
DARKBLUE  = (  0,   0, 255) #1st req: color for the 2nd worm
YELLOW    = (255, 255,   0) #2nd req: color for the new object
DARKGRAY  = ( 40,  40,  40)
BGCOLOR = BLACK
...
def runGame():
    ...
    #1st req: setting start coordinates for 2nd worm
    ...
    #1st req: setting coordinates and direction for 2nd worm
    ...
    #2nd req: start two new object in random places
    ...
    while True: # main game loop
        ...
        #1st req: random moving direction
        ...
        #1st req: if second worm touches the first worm add plus one segment to the second worm
        ...
        #1st req: check if worm has eaten extra object
        ...
        drawWorm(wormCoords)
        drawApple(apple)
        score = (len(wormCoords) - 3) + extraPoints #2nd req: new formula for score
        drawScore(score)
        #1st req: draw the second worm
        if ((time.time() - start) > 20):
            wormCoords1.insert(0, newHead1)
            drawWorm1(wormCoords1)
        #2nd req: appear for 5 seconds every 5 seconds
        if ((time.time() - start) % 10 > 5):
            drawBlueApple(blueApple)
        #2nd req: appear once for 7 seconds
...
def showGameOverScreen():
    ...
    #2nd req: add score to the gameOver screen
    ...
    #3rd req: add start and quit buttons
    buttonFont = pygame.font.SysFont('comicsansms', 25)
    startSurf = buttonFont.render('Start from the beggining', True, BLUE, DARKGRAY)
    quitSurf = buttonFont.render('Quit', True, RED, DARKGRAY)
    ...
    while True:
        if checkForKeyPress():
            pygame.event.get() # clear event queue
            return
        #3rd req: start and quit buttons func
...
#1st req: function for drawing the second worm
